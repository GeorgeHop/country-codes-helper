...
function App() {
    ...
    const [countriesToAdd, setCountriesToAdd] = React.useState();
    const [calculatedCountries, setCalculatedCountries] = React.useState(initialState);
    const [add, setAdd] = React.useState(false);

    const addCountries = (countryCodes, countriesToAdd, locale) => {
        let newCountriesArr = [];
        ...
        countryCodes?.forEach((country) => {
            newCountriesArr?.push({
                ...
                'name': {
                    ...country.name,
                    [locale]: countriesToAdd[country.code]
                }
            })
        });
    ...
    const preparedCountries = React.useMemo(() => {
        let parsed = countriesToAdd?.length > 0 ? JSON.parse(countriesToAdd) : {};
        let calculated = addCountries(calculatedCountries, parsed, short);
        setCalculatedCountries(calculated);
        return calculated;
    },[add]);
        ...
        <div className="App" style={{display: 'flex', flexDirection: 'row', width: '100%'}}>
            ...
            >
                ...
                <input style={{
                    ...
                    border: "none",
                    borderRadius: "3px",
                }} placeholder={'Country short name'} name="short" value={short} onChange={(event) => {
                ...
                <CodeEditor
                    ...
                    onChange={(evn) => setCountriesToAdd(evn.target.value)}
                    padding={15}
                    style={{
                        fontSize: "18px",
                        padding: "10px",
            ...
            >
                <button onClick={() => navigator.clipboard.writeText(JSON.stringify(preparedCountries, null, 2))}>
                    Copy
                </button>
                 <pre style={{
                     color: 'white'
                 }}>
                     {JSON.stringify(preparedCountries, null, 2)}
                 </pre>
            </div>
        </div>
    );
